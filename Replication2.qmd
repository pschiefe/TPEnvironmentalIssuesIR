...
pps_oil <- sim_setx(
  ...
  x = list(
    Oil = 0,
    polity2 = median(Oil$polity2,na.rm = T),
    gdpenl = mean(Oil$gdpenl,na.rm = T),
    jobinsecurity_bdm = mean(Oil$jobinsecurity_bdm,na.rm = T),
    lpop = mean(Oil$lpop,na.rm = T),
    diss_lag = mean(Oil$diss_lag,na.rm = T),
    cowwarl = median(Oil$cowwarl,na.rm = T),
    lmtnest = median(Oil$lmtnest,na.rm = T),
    physint_lag = median(Oil$physint_lag,na.rm = T)
  )
)
